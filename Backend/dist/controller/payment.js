...
const payment = async (req, res) => {
  ...
  const validPassword = await bcrypt_1.default.compare(
    req.body.password,
    user.password
  );
  if (!validPassword) {
  ...
  const sender = user;
  const receiver = await users_1.User.findOne({
    username: recipent.recipent_username,
  });
  if (!sender || !receiver) {
  ...
  try {
    ...
    (0, SendEmail_1.sendMail)(
      sender.email,
      "debit",
      ` your account is debited with Rs. ${recipent.transfer_money} amount on ${sendertransactiondata.timestamp}`
    );
    (0, SendEmail_1.sendMail)(
      receiver.email,
      "credit",
      ` your account is credited with Rs. ${recipent.transfer_money} amount on ${receivertransactiondata.timestamp}`
    );
  ...
  const mailSubject = "Payment Confirmation";
  const mailBody = `Your payment of ${amount} has been received. Thank you!`;

  const mailSent = sendMail(userEmail, mailSubject, mailBody);
  if (mailSent) {
    res.status(200).send("Payment successful and email sent.");
  } else {
    res.status(500).send("Payment successful but email failed to send.");
  }
};
